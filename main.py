...
sql_response_rate = '''
                select
                        ...
                        (select ROUND(SUM(CAST(a.Revenue AS float)), 2)
                            from dbo.Activity AS a
                            where a.UserId = c.UserId and a.Date = c.SendDate
                        ) AS TotRevenue
                    from dbo.Comms as c
                    group by c.UserId, c.OfferId, c.SendDate
                '''

# c.SendDate = a.Date and a.Date <= DATEADD(day, 1, c.SendDate)
def plot_response_rate(sql):
    df = generate_dataframe(sql)
    print(df)

    # plt.show()
    # plt.savefig('Response Rate')
